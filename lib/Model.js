...
Model._alreadyAnInstance = function(type) {
...
Model._parseField = function(desc) {
  var field;
  var container = {};
  if(Model._alreadyAnInstance(desc)) {
    container.typed = desc;
    container.normalized = {type: desc};
    return container;
  } 
  
  if(desc && (Model._alreadyAnInstance(desc.type))) {
...
Model.prototype.validate = function(callback) {
  ...
  for(var prop in this.data) {
    if(this.data.hasOwnProperty(prop)) {
      functs.push(function(cb) {
        self.data[prop].validate(function(e) {
          if(e) {
            errors[prop] = e;
          }
          cb();
  ...
  return callback ? undefined : deferred.promise;
};
